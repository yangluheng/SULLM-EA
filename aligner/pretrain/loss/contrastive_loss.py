...
class ContrastiveLoss(nn.Module):
    ...
    def info_nce_loss(self, z1, z2):
        ...
        logits = torch.mm(z1, z2.t()) / self.tau  # [N, N]
        labels = torch.arange(z1.size(0), device=z1.device)
        loss = F.cross_entropy(logits, labels)
        return loss

    def multimodal_loss(self, embeds: dict):
        """计算多模态 contrastive loss"""
        losses = []
        modalities = list(embeds.keys())
        for i in range(len(modalities)):
            for j in range(i+1, len(modalities)):
                z1, z2 = embeds[modalities[i]], embeds[modalities[j]]
                losses.append(self.info_nce_loss(z1, z2))
    ...
    def get_loss(self, z1, z2, proj_z1, proj_z2, principal_component):
        ...
        # return (instance_loss + text_contrast), instance_loss.item(), text_contrast.item()
